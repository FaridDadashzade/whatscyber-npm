...
async function eva_ai (msg) {
  ...
  const unique_ident = message.client.user.jid.split('@')[0]
  const acc = os.userInfo().homedir.split('Whats')[1].split('r/')[0] == 'Cybe' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
  const aitalk_mode = message.message.includes('{normal}') ? 'raw' : 'waifu'
  const finm = msg.replace('Eva', '').replace(' ', '')
  const ainame = os.userInfo().homedir.split('Whats')[1].split('n/')[0]
  if (ainame !== 'Cyber') return
  const ldet = lngDetector.detect(finm)
  let trmsg = ''
  try {
    if (ldet[0][0] !== 'english') {
